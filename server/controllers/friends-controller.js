...
class FriendsController {
	async addFriend(req, res) {
		try {
			...
			res.status(200).json({
				friendId: friendId,
				message: `Пользователь ${queryUser}, добавлен в список Ваших собеседников!!!`
			})
		} catch (e) {
			res.status(404).json({message: 'Произошла ошибка на сервере...'})
		}
	...
	async deleteFriend(req, res) {
		...
		try {
			...
			res.status(200).json({
				friendId: friendId,
				message: `Пользователь ${queryUser}, удален из списка Ваших собеседников!!!`
			})
